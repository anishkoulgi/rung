...
drawUi = const [ui]
    where
        ...
        rules = B.border $ CO.vBox[C.hCenter $ CO.str "Press arrow keys to scroll this page Press Enter or q to Exit.", B.hBorder,CO.viewport RulesVP T.Vertical $ CO.vBox [
            ...
            CO.str "\n",
            CO.withAttr (A.attrName "underline") (CO.str "Trump Suit:") CO.<+> CO.str "After everyone has been dealt cards, the server chooses a random suit",
            (CO.str "           ") CO.<+> CO.str "to be the trump for each round",
            CO.str "\n",
            CO.str "\n",
